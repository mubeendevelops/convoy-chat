...
const (
	...
	// message.send envelope (content up to 10k UTF-8 chars).
...
type Client struct {
	...
	conn     *gws.Conn
	userID   uuid.UUID
	username string // cached at connect, for user.joined announcements
...
func newClient(srv *Server, conn *gws.Conn, userID uuid.UUID, username string) *Client {
	return &Client{
		srv:      srv,
		conn:     conn,
		userID:   userID,
	...
	for {
		...
		if err != nil {
			if gws.IsUnexpectedCloseError(err, gws.CloseGoingAway, gws.CloseNormalClosure) {
				c.srv.logger.Warn("ws read error", "user_id", c.userID, "error", err)
			}
			return
		}
		c.srv.dispatch(c, data)
	}
}
...
func (c *Client) sendError(code, message string) {
	c.enqueue(errorEvent{Type: eventError, Code: code, Message: message})
}

// enqueue marshals v and best-effort pushes it onto the send queue (never
// blocks). Used for per-client replies originating in the read loop.
